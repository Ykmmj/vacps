...
/** Parse http(s)://host[:port][/path][?query] — no userinfo, no fragments. */
...
struct ClientRequest {
  ...
  std::string body;
  /** 0 = no timeout (not recommended). Default 30s. */
  std::int32_t timeout_ms{30'000};
  /** Reject responses whose body exceeds this (after full read). Default 8 MiB. */
