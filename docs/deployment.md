...
The installer generates a random `BACKEND_ID` (or receives one generated by the Managed Tunnel flow); each Agent consumes only `agent-<BACKEND_ID>`, so all production nodes may share the same Redis database. Prefer a TLS `rediss://` URL; if a Redis provider only offers `redis://`, use it only when the endpoint is private or tightly firewall-restricted. The installer keeps the HTTP API bound to loopback; do not open port 3100 in the host firewall.
