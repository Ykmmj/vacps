...
```bash
pnpm --filter @vps-agent/control-worker exec wrangler kv namespace create OAUTH_KV
```
...
> **Identity-auth migration:** this release deliberately rejects the former shared `BACKEND_SHARED_TOKEN` protocol. After deploying the migration, remove and reinstall every existing VPS Agent with a freshly generated registration Token; an old Agent cannot pass health checks, registration, or telemetry until updated.
