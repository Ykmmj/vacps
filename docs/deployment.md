...
The installer generates a random `BACKEND_ID` (or receives one generated by the Managed Tunnel flow); each Agent consumes only `agent-<BACKEND_ID>`, so all production nodes may share the same TLS Redis database. The installer keeps the HTTP API bound to loopback; do not open port 3100 in the host firewall.
